...
abstract class BaseViewHolder<D, VB : ViewDataBinding> : RecyclerView.ViewHolder {
    ...
    fun getPreviousItem() = if (adapterPosition != 0)
        wholeList?.get(adapterPosition - 1) else null

    open var listLength = if (data is MutableList<*> && !(data as MutableList<*>).isNullOrEmpty())
        (data as MutableList<*>).size - 1 else 0
