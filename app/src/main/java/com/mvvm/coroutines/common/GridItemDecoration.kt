...
class GridItemDecoration(gridSpacingPx: Int, gridSize: Int) : RecyclerView.ItemDecoration() {
    ...
    override fun getItemOffsets(
        ...
        if (itemPosition < mGridSize) outRect.top = 0 else outRect.top = mSizeGridSpacingPx
        if (itemPosition % mGridSize == 0) {
            ...
            if ((itemPosition + 2) % mGridSize == 0) outRect.right = mSizeGridSpacingPx - padding
            else outRect.right = mSizeGridSpacingPx / 2
